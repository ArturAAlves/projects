...
const input = document.querySelector("#amout")
const container = document.querySelector(".lorem-container")
...
form.addEventListener("submit", function (e) {
    ...
    let randomValue = Math.floor((Math.random() * loremText.length))
    console.log(randomValue)

    if (isNaN(value) || value <= 1) {
        container.innerHTML = `<p>${loremText[randomValue]}</p>`
    }
    else {
        let slicer = loremText.slice(0, value)
        // generateLorem(value)
        let print = slicer.map(function (item) {
            console.log(item)
...
//         lorem.push(loremText[i])
//     }
//     let print = lorem.map(function (item) {
//         console.log(item)
//         return `<p> ${item}</p> <br><br>`
//     }).join("")
//     container.innerHTML = print
